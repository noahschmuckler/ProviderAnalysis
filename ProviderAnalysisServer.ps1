...
async function retryDrafts(){el('retry').disabled=true;try{const c=await api('/api/campaign-resume?id='+campaignId,{method:'POST'});lastCampaignJson='';renderCampaign(c);if(c.state!=='Completed')startCampaign(c)}catch(e){alert('Retry failed: '+e.message)}}
async function poll(){
 ...
 try{let c=await api('/api/campaign?id='+campaignId);renderCampaign(c);const ps=c.providers;
  if(!runner&&ps.some(p=>p.reportState==='Prepared'||p.reportState==='Queued')){runner=true;try{await fetch('/api/run-next',{method:'POST'})}finally{runner=false}}
...
$script:JobWatch=@{};$script:KeepAwake=$false;$script:KeepAwakeBroken=$false
...
function Repair-StalledJobs{
 ...
 foreach($f in @(Get-ChildItem $script:Paths.State -Filter 'job-*.json' -File -ErrorAction SilentlyContinue)){
  ...
  elseif(!(Test-ProcessAlive $workerPid)){$reason='the worker process ended without finishing (the computer may have slept or the process was closed)'}
...
function Initialize-AnalysisJobRecovery{$script:RunNext=$false;foreach($f in Get-ChildItem $script:Paths.State -Filter 'job-*.json' -File -ErrorAction SilentlyContinue){$j=Json $f.FullName;if(!$j){continue};$state=[string](Get-P $j 'state' '');$err=[string](Get-P $j 'errorSummary' '');if($state -in @('Starting','Running')){if(Test-ProcessAlive ([int](Get-P $j 'workerPid' 0))){continue};Reset-InterruptedJob $j $f.FullName 'the server was restarted while the report was running'}elseif($state -eq 'Failed' -and $err -like '*ConvertTo-NumberValue*'){$j.state='Prepared';$j.percent=0;$j.stage='Recovered after Draft 4.2 helper/queue repair';Set-P $j 'errorSummary' '';Save-JsonAtomic $f.FullName $j}}}
